...
└─ リンク集                      — occasional-reference sources, not live status (D28); sas0's only
    ...
    │                               sandboxing — D14) and 川の防災情報 (river water-level/flood-forecast
    │                               portal — D20), both MLIT-family, merged into one entry in D56
    ├─ 北海道庁                  — a compact link list: the prefecture's homepage (embedding blocked)
    │                               and 北海道防災ポータル (179-municipality live evacuation/warning/
    │                               river/landslide/volcano status, on one map). Named 北海道 防災情報
    │                               until D56
    ├─ 北海道運輸局               — link; multilingual transport-disruption/weather status. Named
    │                               北海道 旅の安全情報 until D56
    ├─ 北海道開発局               — link; the bureau's own curated portal (river/road/port/weather/
    │                               quake/volcano status), distinct from both 北海道運輸局 and 国土交通省
    ...
    │                               Unchanged name (D56)
    └─ 火山防災協議会             — a single link list, one row per volcano with an established
                                    council, to that council's own evacuation-plan material (the same
                                    9 volcanoes as the root-level 火山 instrument above, complementary
                                    rather than duplicate information — D28). Named 火山 until D56
```
