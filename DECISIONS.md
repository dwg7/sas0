...
This was diagnosed from Brave's documented sandboxing behavior and hasn't been re-confirmed in an actual Brave instance (this repo's tooling doesn't have one) — if it doesn't fully resolve the report, check whether Brave's per-iframe WebGL fingerprinting noise is still visually significant enough to break MapLibre's tile rendering even with `allow-same-origin` granted, which would point back toward the general fingerprinting-protection explanation instead.
