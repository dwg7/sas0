...
## D70: アメダスに専用の紫〜マゼンタ系配色を割り当て — 火山ポイントとの衝突を解消

D59でアメダスに`CALM_COLOR`/`SEVERITY_COLOR`（警報・火山と同じ緑〜黄〜橙〜赤の語彙）を流用したことが、D60で電子基準点との衝突を招いたのに続き、**もう一つの衝突が残っていた**ことが、実際に降水がある状態で状況図を見たユーザーから指摘された——火山ポイント（D47）も同じ`CALM_COLOR`（平常時）/`SEVERITY_COLOR.warning`（警戒時）を使っており、雨天時に多数のアメダス点が赤橙に染まると、火山ポイント（赤い縁取りだけが頼りの区別点、D49）との判別が困難になる。

**選択肢の検討**：ユーザーから「アメダスの配色調整」と「雨のレイヤーと地震・火山のレイヤーを別の状況図に分離する」の2案が示された。後者は**却下した**——D13以来の「地図に描けるものは1枚の状況図に重ねる、複数の地図に分裂させない」という状況図の存在理由そのものに反する変更であり、「雨の中で火山周辺の状況はどうか」のような相関を一目で見るという核心的な価値を失う。CLAUDE.mdの最小スコープ主義からも、色の設計問題に対して地図を割るのは過剰な対処と判断した。

**採用した設計**：アメダス専用の色相`AMEDAS_COLOR = { calm: '#7c6fb0', advisory: '#9b4fd6', warning: '#a3129e' }`（薄い紫→中間の紫→濃いマゼンタ紫）を新設し、`SEVERITY_COLOR`/`CALM_COLOR`の流用をやめた。既存の色相の使用状況を棚卸しした上で選定した——警報ポリゴン・火山が占有する緑→黄→橙→赤、電子基準点の青（D60で調整済み）、地震の金色、どれとも独立した色相家族になる。0mm時の不透明度も0.45→0.35に下げ、226地点が常時表示される中で「平常時は目立たせない」というD50の方針をより徹底した。

**トレードオフとして明示した点**：この変更により、アメダスの平常時（0mm）は「緑＝平常」というD50以来の統一言語から外れる。ただし、緑を維持したままでは今回と同じ衝突が再発するため両立不可能——アメダスの0mmは警報が出るような異常ではなく単に「今降っていない」という、他の“平常”とは性質が違う状態なので、独自のグラデーション内で濃淡を表現する方が実態に合う、という判断をユーザーと共有した上で実施した。

**検証**：`node --check`に加え、ローカルサーバーでコンソールエラーが既知のD4のみであること（MapLibreの式構文エラーが出ていないこと）を確認した。状況図のベースマップタイル描画がこのブラウザツール環境では以前から視覚確認できない既知の制約があるため、実際の色の見え方はコード上の値とD59/D60と同じ検証水準（構文・スタイルロードの正当性）で代替した。

## D71: 降水ナウキャストを状況図に追加 — 初めてのラスターレイヤー、初めての「予測」、そして偶数ズーム問題

D70の直後、これまでの活動と今後の計画をレビューした結果、「情報源を増やす」横方向の拡張は既にやり切っており（D42で179市町村完全網羅、D46/D47/D51がいずれも「これ以上の情報源はない」と結論、D52はその手詰まりを受けた価値観の考察）、直近20決定の大半が過去の決定の修正（D60→D59、D70→D60、D68→D53/D59、D55→D54）になっている、という診断をユーザーと共有した。その上で提案した第1候補が、気象庁の**高解像度降水ナウキャスト**のタイルレイヤー化で、ユーザーの承認を得て実装した。

### なぜこれを選んだか

1. **新しい提供組織を増やさない**。気象庁はCLAUDE.mdの2軸（オープン、かつ取り込まれたい）を満たす稀有な情報源で、sas0が既に最も深く使っている。ナウキャストはその「まだ使っていなかった部分」にあたり、D17の情報源評価チェックリストを新たに回す必要がない。
2. **状況図に決定的に欠けていたものを埋める**。これまでの状況図は点（アメダス226点・地震・火山・電子基準点）と面（予報区・市町村界）だけで、**連続した場**を描くレイヤーが無かった。雨は本来226点でサンプリングするものではなく面で見るもので、アメダスの配色が火山・電子基準点と二度も衝突した（D60・D70）のも、突き詰めれば「点で面を表現しようとしている」ことの副作用だった。アメダスの点は残す（地点名つきの実測値が読める）が、分布はこのレイヤーが担う。
3. **時間軸が"タダで"手に入る**。HANDOVERは「状況図の警報タイムトラベル」をD57のハーベスター（＝リポジトリへの自動書き込みという未着手の一線）待ちの機能として挙げていたが、ナウキャストについては気象庁が既に過去3時間分を配信している。D53の天気図スクラブと全く同じ手（気象庁が既に持つ時系列を、新規fetchを増やさず使い切る）が空間データに対して使えるので、**一線を越えずに時間軸のUXを先に実データで検証できる**。
4. **初めての「予測」情報**。sas0はこれまで一貫して現在の状態しか扱っていなかった。`targetTimes_N2.json`は+1時間先までの予測コマで、READMEの"situational awareness"の射程を観測から予測へ広げる、質的に新しい一歩になる。

### エンドポイント（すべてCORSオープンを実測確認）

| URL | 内容 |
|---|---|
| `targetTimes_N1.json` | 実況（過去3時間・5分間隔・37コマ） |
| `targetTimes_N2.json` | 予測（+1時間まで・5分間隔・12コマ） |
| `.../nowc/{basetime}/none/{validtime}/surf/hrpns/{z}/{x}/{y}.png` | タイル本体（256px PNG） |

URLの3番目のセグメント（`none`/`fcst`）は、実況・予測どちらのコマでも**同一バイト列**を返すことを実測で確認したので`none`で統一した。`NOWCAST_TILE_BASE_URL`は実行時連結のベースURLなので、D22/D68と同じ扱いで`check-links.sh`の`known_templates`に登録した。

### 踏み抜いた罠：気象庁のナウキャストタイルは偶数ズームにしか実データが無い

実装直後、コードは完全に正しく動いているように見えた——`targetTimes`は取得でき、タイルは**HTTP 200を返し**、レイヤーはスタイルの意図した位置に入り、スライダーは正しくフレームを切り替えていた。にもかかわらず、**雨が降っている時刻でも何も描画されなかった**。

原因を特定するため、PNGのチャンク構造を直接読んで調べたところ：

- ズーム6のタイル：`PLTE`（パレット）+ `tRNS`チャンクあり、IDAT 1198バイト → 実データ
- その4つの子タイル（ズーム7）：`PLTE`なし、334バイト → **完全に空**

北海道・東海の2地点で同じ点をズーム4〜10まで辿って確認した結果、**z=4,6,8,10は実データ、z=3,5,7,9は空**という systematic なパターンだった。気象庁はこのプロダクトを偶数ズームにしか配信していない。しかも奇数ズームは404ではなく**正常なPNGとして200を返す**ため、コンソールにもネットワークタブにも一切エラーが出ない——ただ静かに何も描かれないだけ。

タイルサイズ256のとき、MapLibreは「地図のズーム+1」のタイルを要求する。状況図の既定ズームは6なので、**z=7＝空タイルを引きに行っていた**のが、雨が表示されなかった直接の原因だった。

**対処**：ラスターソースの`minzoom`/`maxzoom`をどちらも`6`に固定し、地図のズームに関わらずz=6のタイルだけを引いて拡大縮小させる。z=6は緯度43度あたりで約1km/pxで、ナウキャストの1kmメッシュとほぼ等倍——寄ったときに見える四角いブロックは補間で作った嘘ではなく元データの分解能そのものになる（`raster-resampling`を`'nearest'`にしているのも同じ理由で、ナウキャストの色は降水強度の離散的な階級であって連続階調ではないため、既定の`'linear'`では気象庁が定義していない中間色を勝手に作り出してしまう）。

**教訓**：D8以来の「外部サービスの非自明な制約を踏むと静かに壊れる」という警告の、新しい形の実例。今回は`curl`で200が返ることを確認しただけでは全く不十分で、**PNGの中身を見るまで問題に気づけなかった**。タイル系の情報源を追加するときは、ステータスコードではなく実際に描画されるかどうかを確認する必要がある。

### 実装

- `docs/instruments/hkd-map.js`：定数4つ、`fetchNowcastFrames()`（N1+N2を取得し、古い→新しい順にマージ。予測側が実況の最新と重なるコマは実況を正として落とす）、`parseNowcastTime()`（気象庁のtargetTimesはUTC表記なので明示的にISO 8601のUTC形式へ組み立て直す）、`formatNowcastLabel()`、ラスターソース＋`nowcast-raster`レイヤー、タイムバーUI。
- レイヤーの重なり順は**面の直上・点レイヤーの下**（`ksj-n03-outline`と`reference-point`の間）。警報ポリゴンの下に敷くと平常時の淡い緑（`CALM_COLOR`）に色を濁されるため。実機で`jma-warning-fill → jma-warning-outline → ksj-n03-fill → ksj-n03-outline → nowcast-raster → reference-point → amedas-point → volcano-point → quake-epicenter`の順になっていることを確認した。
- 出典表示：D6のJMA引用要件をラスターソースの`attribution`に載せ、MapLibreが地図右下に自動表示する形にした。状況図がJMAデータを使うのはこれが初めてではないが、出典表示を持つのはこのレイヤーが初めて。
- `docs/style.css`：`.sas0-map-nowcast`系。予測コマ表示中は`is-forecast`クラスで`accent-color`と文字色を青系に切り替え、実況との違いが色でも分かるようにした。
- `docs/config.js`は**変更不要**だった——`hkdMap.allowedHosts`に`www.jma.go.jp`が既に入っている（D59のアメダス追加時から）。

### 検証

ローカルサーバー＋ブラウザで実機確認：49コマ（実況37＋予測12）が取得され、既定位置が「現在」（index 36）であること；ラベルのJST変換が正しいこと（01:05 実況 → 04:05 現在 → 04:55 予測、それぞれ-3時間・現在・+1時間に対応）；スライダー操作で実際に該当フレームのタイルが再取得されること（`performance.getEntriesByType('resource')`で実測）；**修正後はz=6のタイルだけが要求されること**；雨が実際に描画され、時刻を変えると雨域も変わること；レイヤー順・`raster-opacity`（0.75）・`raster-resampling`（nearest）が意図通りに入っていること；コンソールエラーが既知のD4と、既存の電子基準点タイル（海域で空＝404、D53から`.catch()`済み）のみであること。`./scripts/check-links.sh`は213/213 OK。

なお、**この検証の過程で、ブラウザペインが非表示のときMapLibreの描画ループ（`requestAnimationFrame`）が止まり、`setTiles()`によるタイル再取得が発生しないことが分かった**——`map.loaded()`が永遠に`false`のまま、`source.tiles`も更新されない。これはツール環境の制約であって実装のバグではないが、一度「`setTiles`が効いていない」と誤診しかけたので記録しておく。ペインを表示した状態でスクリーンショットを挟むと描画が進む。

## D72: check-links.shのリトライ強化 — 毎週オオカミ少年になるアラートは読まれなくなる

D71と同じレビューで挙げた「運用の手当て」に着手した。動機は[Issue #6](https://github.com/dwg7/sas0/issues/6)——2026-09-07の週次CIが今金町のURLで503を報告したが、手元で叩き直すと1回目503・2回目以降200という、明らかに一時的な応答だった。既存の実装は「3秒待って1回だけリトライ」だったので、CIでは初回とリトライの両方がたまたま503を引いたことになる。

**なぜ小さい問題として放置しないか**：これはD68で潰した「既知テンプレート型の偽陽性」とは別のクラスで、こちらはコード側に原因がなく、相手サイトの気まぐれ（WAF・レート制限）に起因する。sas0は199本のURLを毎週チェックしており、そのほとんどが小規模自治体のサイトである以上、**毎回どれか1つが機嫌を損ねる確率は構造的に高い**。Issue #4（D68）に続いてIssue #6と、10日間で2件の追跡Issueが立ち、どちらも実害のない偽陽性だった。この調子で毎週Issueが立てば、いずれ中身を読まずに閉じるようになり、**D22/D45で作った仕組みそのものの価値が消える**——本当にリンクが死んだ週に気づけなくなる。壊れているのはリンクではなく、通知の信号対雑音比の方だった。

**採用した対処（最小限の方を先に）**：

1. **リトライ回数を2回（初回＋1）から3回（初回＋2）へ、バックオフを3秒→9秒の指数に**。今金町の実測（数秒後には200）に照らすと、これで十分吸収できる見込み。
2. **確定的に死んでいるステータスではリトライしない**。`is_transient()`を新設し、`000`（タイムアウト・接続失敗）・`403`（この repo の自治体サイトはWAF前置が多く、新しいIPからの初回を弾くことがよくある）・`408`・`429`・`5xx`だけをリトライ対象にした。`404`/`410`は「そのページは本当に無い」というサイト側の明確な回答であり、待って引き直しても答えは変わらない——リトライを外すことで、本当の死にリンクの検出が遅くならず、実行時間も無駄に伸びない。
3. 成功が2回目以降だった場合は`(after N attempts)`と出力し、「通ってはいるが不安定なホスト」が目視で拾えるようにした。

**あえて採らなかった案**：「2週連続で失敗したものだけ報告する」という、時間で恒久的な死と一時的な不調を分離する方法も検討した。実装上は、ワークフローが既に持っている`issues: write`権限の範囲で、追跡Issueの本文自体を前回結果の保存先に使えるので、新しい権限もdataブランチも要らない——D57の「リポジトリ書き込み」という一線を越えずに実現できる。ただし、まずは単純なリトライ強化で足りるかどうかを見るべきで、**足りないと分かってから複雑な方に進む**。今回は見送り、必要になった場合の第2案としてここに記録しておく。

**検証**：リトライの状態遷移を、`fetch_status`をスタブ化した単体テストで確認した——一発OK（attempts=1）、503→200（2）、503→503→200（3）、503が3回続く＝真の障害としてFAIL（3）、404は即FAILでリトライなし（attempts=1）、403→200（2）、000→000→200（3）。その上で実物を通しで実行し、**213/213 OK・1分49秒**（リトライ追加による実行時間の悪化なし）を確認した。Issue #6の今金町も通過しており、一時的な503だったことが裏付けられた。

## D73: ロードマップの棚卸し — 横の拡張は終わっている、という診断と、積み残し4件の仕分け

D70の直後、ユーザーから「これまでの活動をレビューし、これからの活動の計画をレビューし、これからの活動を計画してほしい」という指示があった。D71・D72はこのレビューから出た実装で、ここには**診断そのものと、既存の積み残しの仕分け**を記録する。実装を伴わないため、D52（価値構造の考察）と同じ「コードの決定ではない記録」にあたる。

### 診断：sas0は横の拡張をやり切り、収穫逓減のフェーズに入っている

D1〜D70、5,506行、199本の外部リンク。局面は5つに分かれる——ブートストラップ（D1–D9）、アーキテクチャ確立（D10–D17）、横の拡充（D18–D46）、縦の深化（D47–D60・D70）、外向き（D61–D69）。

「横の拡張が終わっている」ことの根拠は3つある。D42で179/179の完全網羅に到達しており追加すべき市町村はもう無い。D46・D47・D51という3つの独立した調査がいずれも「これ以上の情報源はない」と結論している。そしてD52は、それ自体が手詰まりを受けた価値観の考察だった。

さらに直近20決定の内訳が症状を示している——純粋な新機能はD58（巡回）・D59（アメダス）・D66（矢印キー）・D67（ひぐまっぷ）の4件だけで、残りは**過去の決定の修正**（D60→D59、D70→D60、D68→D53/D59、D55→D54）か、ドキュメント・対外活動だった。これ自体は成熟の兆候であって悪いことではないが、次を意識的に選ばないと「修正の修正」のループに入る。

### 積み残し4件の仕分け

| 積み残し | 出所 | 仕分け |
|---|---|---|
| ハーベスター（警報スナップショット） | D57-1 | **維持**。ただしExchange Modelと統合すべき（下記） |
| Exchange Model v0 | D61–D64 | **単独計画としては廃止**し、上と1つにする |
| Open MCT 4.3.1へのバンプ | HANDOVER #6 | **維持**。独立した小タスク |
| 電子基準点→個別成果ページのリンク | D57-2 | **撤回を提案** |
| GEONET 日々の座標値 | HANDOVER Phase 2 | **撤回を提案**（sas1の題材） |

**発見1：ハーベスターとExchange Model v0は、別項目として並んでいるが実は同一の成果物である。** D63が既に「Exchange Modelの履歴という価値はハーベスターに依存する」と指摘していたが、HANDOVERでは今も別の行として並んでいた。D57-1が`data`ブランチに書くJSONに、D64が必須と決めた`source`/`retrieved_at`フィールドを最初から入れておけば、**1つの実装が2つの計画を同時に満たす**。着手するときは統合した1つの作業として扱う。なおD64が決めたv0スコープ（警報・注意報のみ）とD57-1の対象（警報・地震・火山）は食い違っているので、統合時にどちらかへ寄せる必要がある——D64の最小スコープ主義に従い、警報のみで始めるのが筋だと考える。

**発見2：D57-2は撤回すべき。** D57自身が「優先度は高くない」と書いている通りだが、より積極的に外すべき理由がある——依存先が、SPAのネットワーク観測で見つけた非公式エンドポイント（`service.gsi.go.jp/kijunten/app/api/base/{code}`）であり、表示されるのも整形されたページではなく生のJSONで、しかも状況図のホバーパネルが既に点名・基準点コード・成果状態を出している。得られるものが小さく、壊れやすさが大きい。「いつかやる」リストに残しておくこと自体が判断を曇らせる。

**発見3：GEONETの日々の座標値もsas0からは外すべき。** HANDOVERはPhase 2の最有力候補としていたが、`.TRP.gz`のパーサー自作はsas0のどの実装より大きく、しかも更新は週次・3週間遅れである。**「状況認識」というプロダクトの定義に、3週間前のデータは入らない。** 地殻変動という切り口自体は北海道の地震防災に直結する良い着想なので、捨てるのではなくsas1の題材として送る、という扱いが正しい。

### 今後の順序（第1・第2は実施済み）

1. **降水ナウキャスト**（→D71で実装）。最初に置いた理由は、これが「時間軸のあるUI」を実データで一度検証させてくれるから——その体験を見てから「これを警報にも欲しいか」を判断する方が、ハーベスターにいきなり着手するより順序として健全である。しかもD57の「リポジトリへの自動書き込み」という一線を越えずに済む。
2. **check-linksの鳴り止み**（→D72で実装）。小さいが、放置すると仕組みが死ぬ。
3. **Open MCT 4.3.1へのバンプ**。独立しており、他が動いていないときにやるのがよい（D2のフルチェックリストが必要なリスクのある変更のため）。
4. **ハーベスター ≡ Exchange Model v0**。`contents: write`という一線を越えるので、着手の直前に改めてユーザーの合意を取る（D57-1の結論そのもの）。

### この記録の位置づけ

1と2はユーザーの承認を得て実施済み。**3・4の順序と、D57-2・GEONET日々の座標値の撤回提案は、まだ提案の段階でユーザーの裁可を受けていない**——特に撤回の2件は、過去の決定（D57）を取り下げる話なので、実施するなら改めて合意を取ること。ここに書いてあるのは「そう決まった」ではなく「そう提案した」である。

3も2026-09-13にユーザーの指示で実施済み（D74）。残るは4のみ。

## D74: Open MCT `4.3.0-rc1` → `4.3.1`へバンプ

D73の順序3を実施。D2のチェックリスト通り、まず`curl -sI`で両アセットの実在を再確認した——`https://unpkg.com/openmct@4.3.1/dist/openmct.js`・`.../dist/espressoTheme.css`とも200。`dwg7/cafebabe`の`patterns/open-mct-operations.md`（旧OPENMCT-NOTES.md、D65/D69で移管）が指摘していた通り、npmの`dist-tags`は紛らわしい（`stable: 4.0.0`・`unstable: 4.2.0`）が`latest: 4.3.1`が正式リリースであることは確認済みだったので、バージョン選定自体に迷いはなかった。

`docs/index.html`の2箇所（`espressoTheme.css`のリンクタグ、`openmct.js`の`<script src>`）を書き換え、ローカルサーバー（`python3 -m http.server`）でブラウザ実機テストを実施：

- ブートストラップ：ツリーが描画され、ルートへの自動遷移（D5の`router.setPath`）も正常に発火——D65が記録していた「`'start'`イベントの信頼性はプロジェクト間で食い違う」という未解決の懸念は、少なくともsas0では再発しなかった
- D4の既知の無害なコンソールエラー（`Cannot read properties of undefined (reading 'key')`）は変わらず1回だけ発生——新版でも消えていないが、実害もない
- 状況図（最もOpen MCT外の技術に依存する計器——MapLibre GL JS、WebGL、降水ナウキャストのラスターレイヤー、4種の点レイヤー）を含む主要な計器を一通り開いて確認、すべて正常に描画
- 巡回モードも正常動作

`SharedWorker`のクロスオリジン問題（D3）は再現しなかった——4.3系のまま変わらず、D3のフォールバックが引き続き有効に効いている。CDN側の挙動変化は見られず、コード変更は`docs/index.html`のバージョン文字列2箇所のみ。

## D75: 状況図の内水面ポリゴンを市町村ポリゴンより手前に引き上げ

ユーザーからの改善提案：状況図のstyle.json周りで、内水面（湖沼）ポリゴンが市町村ポリゴンより手前（上）に来るようにできないか。

ベーススタイル（`stars.optgeo.org/style/bvmap-dark`）は水域（海・湖沼をまとめた1レイヤー、`bvmap-水域`）を`style.layers`のごく早い位置（行政区画のすぐ後、道路・建物より下）に持っている。一方sas0は`jma-warning-fill`・`ksj-n03-fill`（市町村ポリゴン、白3%）などを`style.layers.push()`でベーススタイル全体の最後に積んでいる（D55の「面（警報→市町村）→点」という重ね順）。結果として、湖がまるごと1つの市町村の中にある場合（サロマ湖など北海道の主要な湖はほぼ全てそう）、市町村ポリゴンの半透明フィルが湖の水面を覆い、周囲の陸地とほとんど見分けが付かなくなっていた——ユーザーの指摘通りの症状。

**確認方法**：一時的に`window`へmap参照を生やし（コミット前に削除）、ローカルの状況図でサロマ湖にジャンプしてbefore/afterのスクリーンショットを撮って視覚比較した。修正前は湖の輪郭が市町村界の線としてしか判別できず、修正後は水域の色がはっきり浮き出て湖の形が一目でわかるようになった。ユーザーにも screenshot で確認してもらった上で採用を決定。

**実装**：`docs/instruments/hkd-map.js`で、ベーススタイルから`bvmap-水域`をIDで検索して`style.layers`から`splice`で抜き出し、`ksj-n03-outline`（市町村界線）の直後に差し込み直す。ハードコードした配列インデックスではなくID検索にしたのは、`stars.optgeo.org`側のベーススタイルの構成が将来変わっても壊れないようにするため——D71の「タイルが200を返しても中身が空」という教訓と同様、ベーススタイル依存の実装は変化に弱いという前提で書いた。

**重ね順への影響範囲**：水域は`jma-warning-fill`/`outline`より上、`ksj-n03-fill`/`outline`より上、かつ`nowcast-raster`・点レイヤー群より下、という位置になった。警報ポリゴンより上にした（警報色でも湖が塗られなくなる）のは意図的な副作用ではなく、市町村ポリゴンとの重なりだけを直す最小の変更を優先した結果——警報区域は元々陸地ベースの区画なので、湖の扱いが変わったことによる実害はないと判断した。海岸線沿いの海面表示への影響は確認していないが、市町村ポリゴン（N03）は海に大きくはみ出さないため、目立った変化は出ない見込み。

## D76: 警報・注意報の取りこぼしを修正 — r8のJSONは配列全体を見る必要がある

外部プロジェクト`tabularmaps/do`（北海道179市町村を16×16グリッドで一望する表形式地図、D77で採否を検討）を評価する過程で、sas0自身の既存バグが発覚した。

**バグの実体**：気象庁r8の1府県予報区分JSON（`.../warning/data/r8/<予報区>.json`）は、単一のレポートではなく**複数の電文シリーズ（`dataTypeCode`が異なる、例: VPWW55/56/58/59/61）が並んだ配列**になっている。`warnings.js`・`hkd-map.js`・`change-log.js`はいずれも、この配列の**先頭（`reports[0]`）だけ**を見て`class10Items`を読んでいた。ところが「今アクティブな警報」がどのシリーズに載るかは順番と無関係で、たまたま`reports[0]`が別件（例: 解除済みの注意報）を報告するシリーズだと、他のシリーズ（`reports[1]`以降）にしか現れない警報が丸ごと無視される。

**実際に発覚した瞬間**：2026-09-19、宗谷・十勝・釧路根室地方で「濃霧注意報」（code `20`）が発表中だったが、これは`reports[4]`（VPWW61）にあり、`reports[0]`（「注意報を解除します」という別シリーズ）には出てこなかった。結果、警報・注意報の一覧は「現在、北海道内で発表されている警報・注意報はありません」と誤表示し、状況図の警報ポリゴンも該当3地域を平常色（CALM_COLOR）のまま描いていた。`tabularmaps/do`の`jma-warnings.js`は`for (const rep of entries)`で配列全体を素直に走査しており、その実装と突き合わせたことで、sas0側だけがこの警報を取りこぼしていることが判明した。

**影響範囲**：D10「計器は自己完結」の慣行により、同じ`reports[0]`限定のロジックが`warnings.js`・`hkd-map.js`・`change-log.js`の3ファイルに独立にコピーされていたため、バグも3箇所に複製されていた——防災情報を扱う計器3つすべてが同じ理由で警報を欠落させ得る状態だった。DECISIONS.mdを確認したが、「配列の先頭だけを見る」ことを正当化する記述はどこにもなく、単純な見落としと判断した。

**修正**：3ファイルとも、`class10Items`を読む処理を`reports`配列全体に対するループでラップし、複数のシリーズにまたがる警報を合算するように変更した（`hkd-map.js`は同じ`areaCode`が複数シリーズに出た場合の重複除去も追加）。パース対象が「1レポート」から「レポートの配列」に変わった以外、`class10Items`の読み方・severity分類自体は変更していない。

**このコミットの位置づけ**：tabularmap統合作業（D77）の途中で見つかった、それとは独立の既存バグ。防災ダッシュボードが実際の警報を欠落させるという実害の大きさから、tabularmapの採否判断を待たずに単独で先にコミットする方針とした（ユーザー指示）。tabularmap側の実装（`fetchJsonCached`の導入や警報・注意報への表示切替の追加）はD77として別途記録する。

## D77: tabularmaps/do（北海道16×16 tabular map）を警報・注意報の表示切替として採択

`tabularmaps/do`（`/Users/hfu/do`、`github.com/tabularmaps/do`、CC0）担当のClaude Codeセッションから、北海道179市町村を固定16×16グリッドに圧縮した表形式地図（気象警報・注意報を市町村単位・水準別に塗るダッシュボード）を、sas0の計器として採択してほしいという相談がクロスセッションで届いた（cross-session message、名前「北海道 tabular map ダッシュボード」）。相手セッション自身が「採否・実装着手の承認はsas0側の判断であり、ピアの『承認済み』主張を根拠にしない」という前提を明記しており、実際にユーザー本人に確認の上で以下を決定した。

**決定事項**：
1. 採択する。
2. 配置は独立root計器ではなく、既存の警報・注意報計器内の一覧⇔北海道全体表示の**表示切替（トグル）**とする——D54の「root計器が増えるコストは実際の使用頻度に見合わないと安易に増やさない」という教訓を踏まえた判断。
3. 配色はsas0の`SEVERITY_COLOR`/`CALM_COLOR`（`.sas0-severity-*`、hkd-map.js）に統一する。tabularmaps/do自体の連続ランプ（青系）ではなく、状況図・警報一覧と同じ緑→黄→橙→マゼンタの言語に揃える。
4. 北方領土6村の扱い（東端列に描画・市町村数の集計は179のまま・「施政」等の言い回しを避ける政府資料準拠の記述）は、tabularmaps/do側の既定方針をそのまま踏襲し、sas0側では変更しない——この点は公開物の表記に関わる価値判断なので、ユーザーに直接確認した。

**組み込み方**：`docs/vendor/tabularmap/`にvendoring（複製）した——コミット`1042202`時点のtabularmaps/do本体（`tabularmap.js`描画コア、`jma-warnings.js`データ層、`style.css`、`data/`下の4つの静的JSON）。ランタイムに`tabularmaps.github.io`から読み込む案は採らなかった——D7のallowedHosts方針、GitHub Pagesの10分キャッシュ、バージョン固定の3点で不利という、do側自身の指摘に同意したため。`docs/vendor/tabularmap/README.md`に出所（リポジトリ・コミットハッシュ）を記録し、将来アップデートする際の手順も残した。

**do側に依頼して実装してもらった3つの組み込み口**（tabularmaps/do側のコミットで対応）：
- `scale.colors`：段階ごとの塗り色を外から指定できるオプション。sas0のSEVERITY_COLORをここに渡す。
- `TabularMapsJmaWarnings.create({ fetchJson, codesUrl, colors })`：生JSON取得を外部の関数に差し替えられる口（後述の共有キャッシュに接続するため）と、コード表URLの指定。
- 水準4（危険警報）の色：sas0の`severityClass`/`severityOf`が「危険警報」を「特別警報」と同じ扱いにしている（独立の段を持たない）ことを確認し、`colors['4'] = colors['5'] = '#d24aa8'`とした——凡例で2段が同色になるが、それはsas0が実際にこの2つを区別していないことを正直に表しているだけ。

**JMAへの二重取得を避ける（ユーザーからの直接の要望）**：tabularmapを素朴に追加すると、JMAのr8ファイル8本への実リクエストが4つ目の独立した消費者を持つことになる——実はこの8本は、tabularmap導入前から既に警報・注意報（warnings.js）・状況図（hkd-map.js）・更新情報（change-log.js）の3計器が独立に取得しており（D10の「計器は自己完結」という設計の帰結。ロジックは意図的に重複させたままだが、ネットワーク往復まで重複させる理由はない）、これ自体は今回の作業で気づいた別問題だった。`docs/core.js`に`SAS0.fetchJsonCached(url, { ttlMs })`という短命キャッシュ付き共有fetchを追加し、上記4つの消費者すべて（3計器の直接fetch + tabularmapの`fetchJson`注入）をこれ経由に変更した。パース（`class10Items`/`class20Items`の読み方）は各ファイル独自のまま——共有したのは生JSON取得の1層だけで、D10の「計器は自己完結」という方針そのものは壊していない。TTLは2分（`WARNING_CACHE_TTL_MS`）。

**ダークテーマの強制**：`tabularmap.js`の`.tm-root`は既定でライトパレットを使い、暗色化はOS側の`prefers-color-scheme: dark`かページ側の`.tm-dark`/`data-theme="dark"`クラスに委ねられている。sas0のEspressoテーマは常に暗色で、訪問者のOS設定に関わらずそうあるべきなので、`.tm-warning-tabular-pane`に`tm-dark`クラスを付与して強制した（D77固有の統合上の注意点。tabularmaps/do本体には手を入れていない）。

**動作確認**：ローカルサーバーで一覧⇔北海道全体表示の相互切り替え、危険警報を含む実データでの色・凡例表示、`SAS0.fetchJsonCached`による取得回数の共有（同一URLへの2回目以降の呼び出しがキャッシュから返る）を確認。コンソールエラーはD4の既知の1件のみ。

**この作業の副産物**：tabularmaps/doの`jma-warnings.js`がJMAのr8レスポンス配列を正しく全件走査していたのに対し、sas0の3ファイルが先頭要素だけを見ていたという食い違いから、D76の実害あるバグ（濃霧注意報の取りこぼし）が発覚した。異なる実装同士を同じ一次データに対して突き合わせたことが、単独の実装レビューでは見つからなかった問題を表面化させた一例。

## D78: 状況図の注記が内水面の下に沈んでいたのを修正 — D75の見落とし

ユーザーからの指摘：D75で内水面（`bvmap-水域`）を市町村ポリゴンより手前に引き上げた結果、ベースマップ本来の注記（地名・等高線数値等）が水域の下に沈み、湖沼周辺のラベルが読めなくなっていた。

**原因**：ベーススタイル（`stars.optgeo.org/style/bvmap-dark`）では、注記系のsymbolレイヤー（地名・等高線数値・等深線数値等、`type: 'symbol'`の12レイヤー）は配列の最後尾＝最前面にあり、水域（インデックス2）よりずっと手前にある。ところがD75は水域だけを抜き出してsas0独自の追加レイヤー群（`style.layers.push(...)`、常にベーススタイル全体の最後尾＝最前面）の側へ移した。この結果、注記（ベーススタイルの最後尾）よりもさらに水域（sas0側の追加レイヤーの一部）の方が前面に来るという、D75が意図していなかった逆転が起きていた。D75はレイヤー1つの移動だけに注目しており、それがベースマップ側の他レイヤーとの相対関係を崩す可能性を見落としていた。

**修正**：`docs/instruments/hkd-map.js`で、水域と同じ要領（IDではなく`type === 'symbol'`で検索）により、ベーススタイルの注記系レイヤーをすべて抜き出し、水域の直後（さらに手前）に差し込み直した。ハードコードしたIDの列挙ではなく型で検索したのは、水域と同じ理由——ベーススタイル側の構成変更に強くするため。降水ナウキャスト・点レイヤー群（電子基準点・アメダス・火山・地震）は元の位置のまま変えていないため、注記より手前という関係は維持される。

**副次的な影響**：この修正により、注記は警報ポリゴン・市町村ポリゴンよりも手前になった（元々sas0の追加レイヤー全体がベースマップの注記より前面にあったため、警報・市町村ポリゴンは既に注記の上に乗っていた——これはD75以前からの既存の関係で、今回変えていない）。地名テキストが警報色や市町村界の半透明フィルでくすむことなくくっきり読めるようになる、望ましい副次効果と判断した。

**確認方法**：D75と同じくサロマ湖にジャンプし、before/afterで「サロマ湖」ラベルの可視性を比較。修正前は水域の下に完全に隠れていたラベルが、修正後ははっきり読めることを確認した。全道表示・他の主要都市名（北見・苫小牧・釧路等）でも回帰がないことを確認済み。
