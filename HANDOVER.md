...
- **Architecture is Open MCT's own browse tree** (D10), following a UX consulting pass for a concrete daily-use case: a Hokkaido regional survey department director who wants to check a handful of disaster-related sources by clicking through a list, not stare at a fixed grid. Every instrument is a single full-screen view, one click away. Current shape:
  ...
  └─ リンク集                      — occasional-reference sources, not live status (D28)
      ...
      │                               47 of Hokkaido's 179 municipalities, one row per subprefecture
...
- **市町村 extended from 38 to 47 — all 35 of Hokkaido's cities (市) now covered (batch 5), plus 札幌市の区 investigated.** Batch 5 targeted "remaining cities" rather than "thin subprefectures" for the first time — the last 9 uncovered cities (夕張・美唄・芦別・赤平・三笠・砂川・歌志内, all 空知; 士別, 上川; 石狩, 石狩), closing out full city coverage; every remaining gap toward 179 is now a town or village. Two more stale-search-result cases (士別市, 歌志内市) confirm that failure mode is now expected on every batch, not occasional. Separately, checked whether 札幌市's 10 wards warrant their own entries — only 豊平区 and 清田区 had independently valuable pages (ward-authored walking maps / district evacuation lists going beyond the two citywide resources already linked); the other 8 wards' own pages just re-link the same citywide content, so weren't added (avoiding low-value padding). See [DECISIONS.md](DECISIONS.md) D30, D31.
...
3. **市町村 has 47 of Hokkaido's 179 municipalities** — all 35 cities (市) are now covered (batch 5, D31); the remaining 132 are all towns (町) and villages (村). Deliberately being extended in repeatable batches rather than all at once (D23–D25, D31) — each batch gets independent URL re-verification, a browser walkthrough, and its own DECISIONS.md entry recording whatever judgment calls that batch's municipalities needed, rather than one large low-context bulk add. Adding the next one is one `config.js` array entry (`regionKey` + the usual fields); the grouped-list rendering (D20) scales to far more rows than the old folder tree did without any structural change needed. Several entries (倶知安町, 旭川市, 千歳市, 伊達市, 富良野市, 石狩市) fall back to a general 防災 page rather than a single hazard-map portal (D18, D23, D24, D31). Two recurring failure modes to actively check for on every future batch, not just react to (D25): (a) a municipality's HTTPS failing with a TLS/SNI mismatch even though the domain otherwise resolves fine (名寄市, 釧路町 — both on the same IP, so likely more Hokkaido municipal sites share this) — needs `allowedProtocols: ['http:']`, no code change; (b) the top search result for a hazard-map page already being stale, or not surfacing the real page at all (網走市, 深川市, 士別市, 歌志内市, plus three more caught pre-emptively in batch 4) — always confirm via the municipality's own site navigation, not the search hit; this has now shown up in every single batch. ~~札幌市's individual 区 checked~~ — resolved, see D30 (2 of 10 wards, 豊平区・清田区, had independently valuable pages; the other 8 only re-link the citywide resources already covered).
