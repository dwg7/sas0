...
- **Architecture is Open MCT's own browse tree** (D10), following a UX consulting pass for a concrete daily-use case: a Hokkaido regional survey department director who wants to check a handful of disaster-related sources by clicking through a list, not stare at a fixed grid. Every instrument is a single full-screen view, one click away. Current shape:
  ...
  └─ リンク集                      — occasional-reference sources, not live status (D28)
      ...
      │                               72 of Hokkaido's 179 municipalities, one row per subprefecture
...
- **市町村 extended from 38 to 72 — all 35 cities (市) and all 15 villages (村) now covered, 10 towns added by population, and 札幌市の区 investigated.** Batch 5 (D31) targeted "remaining cities" rather than "thin subprefectures" for the first time — the last 9 uncovered cities, closing out full city coverage. Batch 6 (D32) picked the 10 largest-population not-yet-covered towns regardless of subprefecture, sourced against Hokkaido's own 令和7年国勢調査速報 population figures rather than guessed; found 2 more HTTPS/SNI-mismatch cases and 3 more stale-search-result cases, confirming both failure modes are now expected on every batch. Batch 7 (D33) covered all 15 of Hokkaido's villages (村) in one pass (few enough to do at once) — notably found *zero* of the two known failure modes this time, but surfaced a new domain-pattern gotcha (several villages use `.lg.jp` rather than the more common `vill.*.hokkaido.jp` pattern, and two use fully custom domains). The remaining 107 municipalities toward 179 are all towns. Separately, checked whether 札幌市's 10 wards warrant their own entries — only 豊平区 and 清田区 had independently valuable pages; the other 8 only re-link citywide content already covered, so weren't added. See [DECISIONS.md](DECISIONS.md) D30, D31, D32, D33.
...
3. **市町村 has 72 of Hokkaido's 179 municipalities** — all 35 cities (市) are covered (batch 5, D31); all 15 villages (村) are covered (batch 7, D33); batch 6 (D32) added 10 towns by population. The remaining 107 are all towns (町). Deliberately being extended in repeatable batches rather than all at once (D23–D25, D31–D33) — each batch gets independent URL re-verification, a browser walkthrough, and its own DECISIONS.md entry recording whatever judgment calls that batch's municipalities needed, rather than one large low-context bulk add. Adding the next one is one `config.js` array entry (`regionKey` + the usual fields); the grouped-list rendering (D20) scales to far more rows than the old folder tree did without any structural change needed. Several entries (倶知安町, 旭川市, 千歳市, 伊達市, 富良野市, 石狩市, 真狩村, 中札内村, 音威子府村) fall back to a general 防災 page rather than a single hazard-map portal (D18, D23, D24, D31, D33). Two recurring failure modes to actively check for on every future batch, not just react to (D25): (a) a municipality's HTTPS failing with a TLS/SNI mismatch even though the domain otherwise resolves fine (名寄市, 釧路町, 七飯町, 美幌町 — 名寄市/釧路町 share an IP, so likely more Hokkaido municipal sites share this) — needs `allowedProtocols: ['http:']`, no code change; (b) the top search result for a hazard-map page already being stale, or not surfacing the real page at all (網走市, 深川市, 士別市, 歌志内市, 当別町, 森町, 斜里町, plus three more caught pre-emptively in batch 4) — always confirm via the municipality's own site navigation, not the search hit; this has shown up in every single batch since D22, no longer worth flagging as a surprise, just a standard step. ~~札幌市's individual 区 checked~~ — resolved, see D30 (2 of 10 wards, 豊平区・清田区, had independently valuable pages; the other 8 only re-link the citywide resources already covered).
