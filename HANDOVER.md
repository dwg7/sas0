...
- **Architecture is Open MCT's own browse tree** (D10), following a UX consulting pass for a concrete daily-use case: a Hokkaido regional survey department director who wants to check a handful of disaster-related sources by clicking through a list, not stare at a fixed grid. Every instrument is a single full-screen view, one click away. Current shape:
  ...
  └─ リンク集                      — occasional-reference sources, not live status (D28)
      ...
      │                               82 of Hokkaido's 179 municipalities, one row per subprefecture
...
- **市町村 extended from 38 to 82 — all 35 cities (市) and all 15 villages (村) now covered, 20 towns added by population across two batches, and 札幌市の区 investigated.** Batch 5 (D31) targeted "remaining cities" rather than "thin subprefectures" for the first time — the last 9 uncovered cities, closing out full city coverage. Batches 6 and 8 (D32, D34) each picked the 10 largest-population not-yet-covered towns regardless of subprefecture, sourced against Hokkaido's own 令和7年国勢調査速報 population figures rather than guessed. Batch 7 (D33) covered all 15 of Hokkaido's villages (村) in one pass — notably found *zero* of the two known failure modes that batch, but surfaced a domain-pattern gotcha (several municipalities use `.lg.jp` or a fully custom domain rather than the more common `town/vill.*.hokkaido.jp` pattern) that batch 8 confirmed applies to towns too (長沼町, 東川町, 厚岸町). Batch 8 also hit a first: a page (東川町) whose `<title>` is a generic sitewide string rather than page-specific (JS-driven site), needing body-content verification instead — and a `scripts/check-links.sh` false-FAIL that turned out to be the URL itself flaking between 200/404 on repeated identical requests (verified by hand, not a real dead link). The remaining 97 municipalities toward 179 are all towns. Separately, checked whether 札幌市's 10 wards warrant their own entries — only 豊平区 and 清田区 had independently valuable pages; the other 8 only re-link citywide content already covered, so weren't added. See [DECISIONS.md](DECISIONS.md) D30–D34.
