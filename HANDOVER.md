...
- **Architecture is Open MCT's own browse tree** (D10), following a UX consulting pass for a concrete daily-use case: a Hokkaido regional survey department director who wants to check a handful of disaster-related sources by clicking through a list, not stare at a fixed grid. Every instrument is a single full-screen view, one click away. Current shape:
  ...
  └─ リンク集                      — occasional-reference sources, not live status (D28)
      ...
      │                               92 of Hokkaido's 179 municipalities, one row per subprefecture
...
- **市町村 extended from 38 to 92 — all 35 cities (市) and all 15 villages (村) now covered, 30 towns added by population across three batches, and 札幌市の区 investigated.** Batch 5 (D31) targeted "remaining cities" rather than "thin subprefectures" for the first time — the last 9 uncovered cities, closing out full city coverage. Batches 6, 8, and 9 (D32, D34, D35) each picked the 10 largest-population not-yet-covered towns regardless of subprefecture, sourced against Hokkaido's own 令和7年国勢調査速報 population figures rather than guessed. Batch 7 (D33) covered all 15 of Hokkaido's villages (村) in one pass — surfaced a domain-pattern gotcha (several municipalities use `.lg.jp` or a fully custom domain rather than `town/vill.*.hokkaido.jp`) that later batches confirmed applies to towns too. Batch 8 hit a page whose `<title>` is a generic sitewide string (JS-driven site, needed body-content verification) and a `scripts/check-links.sh` false-FAIL from a URL that flakes between 200/404 on identical requests (verified by hand, not a real dead link — same category as the script's own documented caveat). Batch 9 found a 5th HTTPS/SNI-mismatch town (洞爺湖町) and a case where a town's marketing site and its administrative site live on entirely different domains (標茶町). The remaining 87 municipalities toward 179 are all towns. Separately, checked whether 札幌市's 10 wards warrant their own entries — only 豊平区 and 清田区 had independently valuable pages; the other 8 only re-link citywide content already covered, so weren't added. See [DECISIONS.md](DECISIONS.md) D30–D35.
