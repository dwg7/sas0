window.SAS0_CONFIG = {
  ...
  // ひぐまっぷ（ダッピスタジオ合同会社運営）。北海道内の複数市町村（美幌町・
  // 千歳市・釧路市・旭川市等）と連携し、各自治体が受け付けたヒグマ出没・
  // 被害情報を地図で公開する仕組み——情報源は自治体自身であり、D17の
  // 「公的機関のデータ」という軸に合う。実データ配信元（`/recent/reportsJson`）
  // にCORSヘッダーが無いことをcurlで確認済みのため、計器化はできずリンクで
  // 案内する。See DECISIONS.md D67, issue #5.
  higumap: {
    title: 'ひぐまっぷ',
    description:
      '北海道内の市町村と連携したヒグマ出没・被害情報マップ「ひぐまっぷ」（ダッピスタジオ合同会社運営）を新しいタブで開きます。',
    url: 'https://higumap.info/recent'
  },
