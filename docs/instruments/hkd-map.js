(function () {
  ...
  async function render(container) {
    ...
    // 内水面（湖沼等）はベーススタイル側で「bvmap-水域」という1レイヤーに
    // 海と一緒にまとめられており、レイヤー順としてはもともと市町村ポリゴンより
    // 大きく下（道路・建物より手前だが行政区画の直後）にある。sas0側で
    // 市町村ポリゴン・警報ポリゴンをその上から重ねているため、湖の面積が
    // まるごと市町村の中にある場合（サロマ湖など）、湖の水色がこれらの
    // 半透明フィルで沈み、周囲の陸地との区別が付きにくくなる。市町村ポリゴン
    // より手前（上）に引き上げることで、湖の水面が地図上でも認識できるように
    // する（D75）。IDでの検索にしているのは、ベーススタイル側のレイヤー順が
    // 将来変わってもここが壊れないようにするため。
    const waterLayerIndex = style.layers.findIndex((layer) => layer.id === 'bvmap-水域');
    const waterLayer = waterLayerIndex >= 0 ? style.layers.splice(waterLayerIndex, 1)[0] : null;

    // ベーススタイル本来の注記（地名・等高線数値等のsymbolレイヤー、常に
    // 配列の最後尾＝最前面にある）は、水域がインデックス2という手前に
    // 沈んでいた頃は当然その上に描かれていた。ところが上のwaterLayerを
    // sas0独自スタックの側（配列の最後尾＝ベーススタイル全体より前面）へ
    // 移したことで、今度は注記の方が水域の下に沈んでしまった——D75は
    // 「水域が市町村ポリゴンに沈む」問題だけを直し、この副作用は見落として
    // いた。type:'symbol'で検索して同様に抜き出し、水域の直後（さらに手前）
    // に差し込み直すことで、「水域の上に注記」という元の関係を復元する
    // （D78）。ハードコードしたIDの列挙ではなく型で検索しているのは、
    // waterLayerと同じ理由——ベーススタイル側の構成変更に強くするため。
    const labelLayers = [];
    for (let i = style.layers.length - 1; i >= 0; i -= 1) {
      if (style.layers[i].type === 'symbol') {
        labelLayers.unshift(style.layers.splice(i, 1)[0]);
      }
    }

    // MapLibreはstyle.layers配列の後ろにあるものほど上に描画される。
    // 重なり順は下から：地域の面（警報ポリゴン→市町村ポリゴン→内水面→
    // ベースマップの注記）→電子基準点→アメダス→火山→地震、の順に積む
    // （＝画面上での見え方は
